...
if nargin<3
    N = 51; %change to number of images in stack, can also change in function call)
end
...
if(~isempty(dirlist))
    ...
    skel_ini = Skeleton3D(bin,cells); %this call runs fine, no bugs just long
    save skel_ini skel_ini
    save cells cells
    clear bin
    
    % convert voxel skeleton to network graph and back to cleaned skeleton
    [A,node,link,skel] = OCY_run_Skel2Graph3D(skel_ini,cells,5); %runs fine, just long
    clear skel_ini
